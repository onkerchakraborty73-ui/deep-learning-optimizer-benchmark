...
class HybridScheduler:
    """CosineAnnealingLR for both native optimizers in MuonHybrid."""
    def __init__(self, optimizer, t_max):
        self.muon_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer.muon, T_max=t_max
        )
        self.adamw_scheduler = (
            torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer.adamw, T_max=t_max
            )
            if optimizer.adamw is not None else None
        )

    def step(self):
        self.muon_scheduler.step()
        if self.adamw_scheduler is not None:
...
def run_experiment(
    ...
    if isinstance(optimizer, MuonHybrid):
        scheduler = HybridScheduler(optimizer, args.epochs)
    else:
    ...
    if isinstance(optimizer, MuonHybrid):
        print(
            f"Muon params: {optimizer.muon_param_count:,} | "
            f"AdamW fallback params: {optimizer.fallback_param_count:,}"
        )
