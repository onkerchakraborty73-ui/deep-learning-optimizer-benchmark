    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                exp_avg.mul_(beta2)
                exp_avg.add_(grad, alpha=1.0 - beta2)
...
class ShampooLite(torch.optim.Optimizer):
    """
    Diagonal Shampoo-style adaptive optimizer.

    This is intentionally labeled ShampooLite because it is NOT the full
...
def make_optimizer(name, model, lr, weight_decay):
    ...
    if name == "muon":
        ...
        for name_, p in model.named_parameters():
            ...
            # Muon is appropriate for matrix parameters.
            if (
                p.ndim == 2
                and "bias" not in name_.lower()
                and "norm" not in name_.lower()
            ):
        ...
        try:
            ...
            raise RuntimeError(
                f"Muon initialization failed: {exc}"
            ) from exc
